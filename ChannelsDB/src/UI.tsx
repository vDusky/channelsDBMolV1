...
export class UI extends React.Component<{ plugin: Context }, { isLoading?: boolean, error?: string, data?: any, isWaitingForData?: boolean, apiStatus: number|undefined }> {
    ...
    load() {
        ...
        // loadData(this.props.plugin!, this.currentProteinId, channelsURL, this.subDB) //'channels.json'
        //     .then(data => {
        //         //console.log("loading done ok");
        //         let _data = (this.props.plugin.context.select("channelsDB-data")[0] as Bootstrap.Entity.Data.Json).props.data as ChannelsDBData;
        ...
        //     });
        this.props.plugin.loadData(channelsURL, this.currentProteinId.toLowerCase(), this.subDB)
            .then(data => {
                if ((data as any).Error !== void 0){
                    this.setState({ isLoading: false, error: (data as any).Error as string });
                } else {
                    this.setState({ isLoading: false, data: data });
                    showDefaultVisuals(this.props.plugin, data.Channels as ChannelsDBData, 2);
                }
            })
            .catch(e => {
                console.log(`ERR on loading: ${e}`);
...
export class Channel extends React.Component<{state:State, channel: any }, { isVisible: boolean, isWaitingForData:boolean }> {
    ...
    render() {
        let c = this.props.channel as Tunnel;
        ...
        if(annotations!==null && annotations !== void 0){
            ...
            return <Renderable annotations={annotations} label={<span><b><a onClick={this.selectChannel.bind(this)}>{annotation.text}</a></b>, Length: {len} Å</span>} element={c} toggle={showChannelVisuals} {...this.props.state} />
        }
        else{
            return <Renderable label={<span><b><a onClick={this.selectChannel.bind(this)}>{c.Type}</a></b>, {`Length: ${len} Å`}</span>} element={c} toggle={showChannelVisuals} {...this.props.state} />
        }
    }
