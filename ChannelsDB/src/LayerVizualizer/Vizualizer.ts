...
import { Tunnel, ColorPaletteFunctionSettings, ContextAwareObject, ColorMixer, TextBox, CurlyBrackets, ArrowHeadLine } from "./Backend";
import { Axis as Axisbackend } from "./Backend";
import { Numbers } from "../CommonUtils/Numbers";
import Tabs from "../CommonUtils/Tabs";
import { Paintable, Positionable, Axis } from "./Components";
...
export class Vizualizer{      
    ...
    private onWindowResize(e: Event){
        ...
        if(canvas === void 0 || context === void 0 || !this.isElementVisible(canvas) || !Tabs.isActive("left-tabs","1")){
            return;
        }
    ...
    private getComponentsPositioning(){
        ...
        let positioning = {
            ...
            verticalAxis_1: new Axis(),
            verticalAxis_2: new Axis(),
    ...
    public vizualize(renderHitboxes = true){
        ...
        maxDistance = Numbers.roundToDecimal(maxDistance,1); // round to 1 decimal
        ...
        */
        ...
        toRender.push({
            drawable: new Axisbackend(0,this.maxX,context,10,true,20,1.6),
            bounds: positioning.horizontalAxis.toBounds(canvasWidth,canvasHeight)   
        });
        
        //Prepare vertical axis
        toRender.push({
            drawable: new Axisbackend(0,this.getMaxY(this.maxY),context,this.verticalAxisChunkCount,false,30,1.6),
            bounds: positioning.verticalAxis_1.toBounds(canvasWidth,canvasHeight)   
        });
        toRender.push({
            drawable: new Axisbackend(0,this.getMaxY(this.customMaxY),context,this.verticalAxisChunkCount,false,30,1.6),
        ...
        */
        ...
        toRender.push({
            drawable: new TextBox(`${Numbers.roundToDecimal(dTminVal,2)}`,context,"small","left"),
            bounds: positioning.dColorMixerLeftLabel.toBounds(canvasWidth,canvasHeight)   
        });
        // Label on top right side of color mixer for default tunnel
        toRender.push({
            drawable: new TextBox(`${Numbers.roundToDecimal(dTmaxVal,2)}`,context,"small","right"),
        ...
        */
        ...
        toRender.push({
            drawable: new TextBox(`${Numbers.roundToDecimal(cTminVal,2)}`,context,"small","left"),
            bounds: positioning.cColorMixerLeftLabel.toBounds(canvasWidth,canvasHeight)   
        });
        // Label on bottom right side of color mixer for customizable tunnel
        toRender.push({
            drawable: new TextBox(`${Numbers.roundToDecimal(cTmaxVal,2)}`,context,"small","right"),
